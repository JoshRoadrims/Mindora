...
export default function Messages() {
  ...
  const loadThread = useCallback((professionalId) => {
    api
      .getMessageThread(professionalId)
      .then(setMessages)
      .catch((err) => setError(err.message))
  }, [])
  ...
  const selectedConversation = conversations?.find((c) => c.professionalId === selectedId)
      ...
      <div className="p-8 max-w-5xl">
        ...
        {conversations && conversations.length === 0 && (
          <Card className="text-center py-16 text-ink-500 text-sm">
            No conversations yet — book an appointment with a professional to start messaging them.
          </Card>
        )}
            ...
            <Card className="p-0 overflow-y-auto md:col-span-1">
              {conversations.map((c) => (
                <button
                  ...
                  onClick={() => setSelectedId(c.professionalId)}
                  className={`w-full text-left px-4 py-3 border-b border-ink-100 last:border-0 transition-colors ${
                    selectedId === c.professionalId ? 'bg-teal-50' : 'hover:bg-ink-50'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-navy-800 text-sm">{c.professionalName}</p>
                    {c.unreadCount > 0 && (
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                    ...
                    {messages?.map((m) => (
                      <div key={m.id} className={`flex ${m.senderRole === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        >
                          {m.content}
                          <p className={`text-[10px] mt-1 ${m.senderRole === 'user' ? 'text-navy-200' : 'text-ink-400'}`}>
